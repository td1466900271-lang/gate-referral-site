...
IMAGES = {
    "zh-cn": {
        "file": "market-brief-2026-08-05-zh-cn.svg",
        "tag": f"{DATE} · 全球市场日报",
        "title": "全球风险共振，AI与存储强力修复",
        "subtitle": "美股与亚洲股市同步走高，油价急跌释放估值压力；盈利兑现继续决定AI主线。",
        "cards": [("S&P 500", "+1.8%", "创历史新高"), ("TAIEX / TSMC", "+3.2 / +3.5", "亚洲午间变化"), ("TraderXYZ", "$4.70B", "前十成交增 27%")],
        "box1": ("3 个关键观察", ["1. TSMC 2400 能否站稳？", "2. 存储反弹能否获现货确认？", "3. 原油 78 与非农如何定价？"]),
        "box2": ("风险框架", ["盈利与油价支持风险偏好。", "急涨仍含空头回补与杠杆。", f"GateAffiliate · 邀请码 {CODE}"]),
        "bottom": "核心：AI资本开支与盈利共振，但高波动资产仍需订单、现金流和现货成交验证。",
    },
    "zh-hant": {
        "file": "market-brief-2026-08-05-zh-hant.svg",
        "tag": f"{DATE} · 全球市場日報",
        "title": "全球風險共振，AI與記憶體強力修復",
        "subtitle": "美股與亞洲股市同步走高，油價急跌釋放估值壓力；盈利兌現繼續決定AI主線。",
        "cards": [("S&P 500", "+1.8%", "創歷史新高"), ("TAIEX / TSMC", "+3.2 / +3.5", "亞洲午間變化"), ("TraderXYZ", "$4.70B", "前十成交增 27%")],
        "box1": ("3 個關鍵觀察", ["1. TSMC 2400 能否站穩？", "2. 記憶體反彈能否獲現貨確認？", "3. 原油 78 與非農如何定價？"]),
        "box2": ("風險框架", ["盈利與油價支持風險偏好。", "急漲仍含空頭回補與槓桿。", f"GateAffiliate · 邀請碼 {CODE}"]),
        "bottom": "核心：AI資本開支與盈利共振，但高波動資產仍需訂單、現金流和現貨成交驗證。",
    },
    "en": {
        "file": "market-brief-2026-08-05-en.svg",
        "tag": f"{DATE} · Global Market Brief",
        "title": "Global risk rally lifts AI and memory",
        "title_size": 46,
        "subtitle": "U.S. and Asian stocks rise together as oil plunges; proven earnings continue to decide AI leadership.",
        "cards": [("S&P 500", "+1.8%", "record close"), ("TAIEX / TSMC", "+3.2 / +3.5", "Asia midday change, %"), ("TraderXYZ", "$4.70B", "top-ten volume +27%")],
        "box1": ("3 key checks", ["1. Can TSMC hold 2400?", "2. Does cash confirm memory's rebound?", "3. How do $78 oil and payrolls price?"]),
        "box2": ("Risk Frame", ["Earnings and oil support risk appetite.", "The surge still includes shorts and leverage.", f"GateAffiliate · invite code {CODE}"]),
        "bottom": "Bottom line: AI capex and earnings align, but volatile assets still need orders, cash flow and cash-market confirmation.",
        "bottom_width": 78,
    },
    "ru": {
        "file": "market-brief-2026-08-05-ru.svg",
        "tag": f"{DATE} · Обзор рынка",
        "title": "Ралли риска поднимает AI и память",
        "title_size": 42,
        "subtitle": "США и Азия растут вместе, нефть резко падает; лидеров AI определяет реальная прибыль.",
        "subtitle_width": 42,
        "cards": [("S&P 500", "+1.8%", "новый рекорд"), ("TAIEX / TSMC", "+3.2 / +3.5", "изменение в Азии, %"), ("TraderXYZ", "$4.70B", "оборот +27%")],
        "box1": ("3 ключевых сигнала", ["1. Удержит ли TSMC уровень 2400?", "2. Подтвердит ли спот рост памяти?", "3. Как нефть $78 и занятость повлияют?"]),
        "box2": ("Рамка риска", ["Прибыль и нефть поддерживают риск.", "В скачке есть шорты и плечо.", f"GateAffiliate · код {CODE}"]),
        "bottom": "Итог: капзатраты и прибыль AI растут вместе, но нужны заказы, поток и подтверждение спотом.",
        "bottom_width": 45,
    },
}
